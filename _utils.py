...
def predict(net, dataloader, out_idx=None, use_sign=False, verbose=True, usage=""):#false 实数
...
def rename_output(args):
    if os.path.exists(f"./output/{args.backbone}"):
        ...
        print(f"rename previous output-old {args.backbone} -> {args.backbone}-m{m_next}")
...
def validate(args, query_loader, dbase_loader, early_stopping, epoch, **kwargs):
    ...
    print('epoch ecodetime:', encode_time)
    ...
    # if map_v.item() == early_stopping.best_map:
    #     Save_mat(
    #         epoch=epoch,
